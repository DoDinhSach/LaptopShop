...
public class DonHangDao {
    ...
    private final GiamGiaDao giamGiaDao;
    private String lastCheckoutError;

    public DonHangDao(Context ctx) {
        dbHelper = new DBHelper(ctx);
        gioHangDao = new GioHangDao(ctx);
        sanPhamDao = new SanPhamDao(ctx);
        historyDao = new InventoryHistoryDao(ctx);
        giamGiaDao = new GiamGiaDao(ctx);
    }
    ...
    public long checkout(long userId, CheckoutInfo info) {
        ArrayList<GioHangItem> items = gioHangDao.getCartItems(userId);
    ...
    public long checkout(long userId, CheckoutInfo info, List<Long> selectedCartItemIds) {
        ArrayList<GioHangItem> items = gioHangDao.getCartItemsByIds(userId, selectedCartItemIds);
    ...
    public long checkoutSingleProduct(long userId, long productId, int quantity, String storage, String color, CheckoutInfo info) {
        GioHangItem item = buildSingleProductCartItem(productId, quantity, storage, color);
        if (item == null) {
            setLastCheckoutError("Không tìm thấy sản phẩm");
            return -1;
        }

        ArrayList<GioHangItem> items = new ArrayList<>();
        items.add(item);
        return checkoutWithItems(userId, info, items, false, null);
    }

    public int previewSingleProductTotal(long productId, int quantity) {
        GioHangItem item = buildSingleProductCartItem(productId, quantity, "", "");
        return item == null ? 0 : item.thanhTien();
    }

    private long checkoutWithItems(long userId, CheckoutInfo info, ArrayList<GioHangItem> items,
        ...
        if (discountCode != null) {
            com.example.laptopshop.data.model.GiamGia voucher = giamGiaDao.getByCode(discountCode);
            discountAmount = CheckoutInfo.calculateDiscount(voucher, subtotal);
        }
        ...
        try {
            ...
            orderValues.put(DBHelper.COL_O_REFUND_STATUS, DonHang.REFUND_STATUS_NONE);
            ...
            setLastCheckoutError("Lỗi đặt hàng: " + e.getMessage());
            return -1;
        } finally {
            db.endTransaction();
        }
    ...
    public DonHang getOrderById(long orderId) {
    ...
    public ArrayList<DonHang> getOrdersByUser(long userId) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
    ...
    public ArrayList<DonHang> getAllOrders() {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        ArrayList<DonHang> list = new ArrayList<>();
    ...
    public ArrayList<DonHangItem> getOrderItems(long orderId) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
    ...
    public boolean updateOrderStatus(long orderId, String newStatus) {
        ...
        db.beginTransaction();
        try {
            DonHang order = getOrderByIdInTransaction(db, orderId);
    ...
    private Map<Long, Double> getWeightedAverageCostByProduct(SQLiteDatabase db) {
        ...
        Cursor c = db.rawQuery(
                ...
                new String[]{com.example.laptopshop.data.model.PhieuNhap.STATUS_COMPLETED}
        );
        while (c.moveToNext()) {
    ...
    public ArrayList<String> getAllowedNextOrderStatuses(DonHang order) {
    ...
    public ArrayList<String> getAllowedNextPaymentStatuses(DonHang order) {
        ArrayList<String> allowed = new ArrayList<>();
    ...
    public boolean canTransitionOrder(DonHang order, String newStatus) {
    ...
    public boolean canTransitionPayment(DonHang order, String newStatus) {
    ...
    private int reconcileExpiredTransferOrders(SQLiteDatabase db, long now) {
        ...
        for (Long orderId : expiredOrderIds) {
            DonHang order = getOrderByIdInTransaction(db, orderId == null ? -1L : orderId);
            ...
            values.put(DBHelper.COL_O_PAYMENT_DEADLINE, 0L);
            values.put(DBHelper.COL_O_REFUND_STATUS, DonHang.REFUND_STATUS_NONE);
    ...
    private boolean shouldExpireTransferOrder(DonHang order, long now) {
        return order != null
    ...
    private ContentValues buildOrderStatusUpdateValues(DonHang order, String newStatus) {
        ContentValues values = new ContentValues();
        ...
        if (OrderStatus.STATUS_DA_HUY.equals(newStatus)) {
            ...
            if (isBankTransfer(order) && PaymentStatus.STATUS_DA_THANH_TOAN.equals(order.trangThaiThanhToan)) {
                values.put(DBHelper.COL_O_REFUND_STATUS, DonHang.REFUND_STATUS_CHO_HOAN_TIEN);
    ...
    private boolean handleCancelledOrderStockAfterUpdate(SQLiteDatabase db, long orderId, ContentValues values, DonHang order) {
    ...
    private String getCancelReason(DonHang order) {
        if (isBankTransfer(order) && PaymentStatus.STATUS_DA_THANH_TOAN.equals(order.trangThaiThanhToan)) {
    ...
    private String buildRefundNote(DonHang order) {
    ...
    private DonHang getOrderByIdInTransaction(SQLiteDatabase db, long orderId) {
        Cursor c = db.rawQuery(
    ...
    private DonHang readOrder(Cursor c) {
    ...
    private GioHangItem buildSingleProductCartItem(long productId, int quantity, String storage, String color) {
    ...
    private boolean reserveStockForItem(SQLiteDatabase db, GioHangItem item) {
    ...
    private ArrayList<DonHangItem> getOrderItemsInTransaction(SQLiteDatabase db, long orderId) {
        ...
        while (c.moveToNext()) {
            DonHangItem it = new DonHangItem();
            it.id = c.getLong(0);
    ...
    private boolean restoreStockForOrder(SQLiteDatabase db, long orderId, String note) {
        ...
        ArrayList<DonHangItem> items = getOrderItemsInTransaction(db, orderId);
    ...
    private String getCancelRestockNote(DonHang order, boolean expiredTransfer) {
        if (expiredTransfer) {
