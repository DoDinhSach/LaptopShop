...
public class NguoiDungDao {
    ...
    public boolean dangKyKhachHang(String fullname, String username, String password) {
        ...
        v.put(DBHelper.COL_PASSWORD, maHoaMatKhau(password));
    ...
    public NguoiDung dangNhap(String username, String password) {
        ...
        if (c.moveToFirst()) {
            ...
            if (khopMatKhau(password, storedPassword)) {
                u = docNguoiDung(c);
                nangCapMatKhauNeuCan(db, u.id, storedPassword, password);
            }
        }
        c.close();
        return u;
    ...
    public boolean capNhatHoTen(long id, String fullname) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues v = new ContentValues();
        v.put(DBHelper.COL_FULLNAME, fullname);
    ...
    public boolean doiMatKhau(long id, String oldPassword, String newPassword) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        Cursor c = db.rawQuery(
        ...
        if (!khopMatKhau(oldPassword, storedPassword)) {
            return false;
        }

        ContentValues v = new ContentValues();
        v.put(DBHelper.COL_PASSWORD, maHoaMatKhau(newPassword));
        return db.update(
                DBHelper.TBL_USERS,
                v,
    ...
    public ArrayList<NguoiDung> layKhachHang(String keyword) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
    ...
    public boolean adminUpdateCustomer(long id, String fullname, String newPasswordOrNull) {
        ...
        if (newPasswordOrNull != null && !newPasswordOrNull.trim().isEmpty()) {
            v.put(DBHelper.COL_PASSWORD, maHoaMatKhau(newPasswordOrNull.trim()));
        }
    ...
    private NguoiDung docNguoiDung(Cursor c) {
    ...
    private boolean khopMatKhau(String rawPassword, String storedPassword) {
        if (rawPassword == null || storedPassword == null) {
            return false;
        }
        return storedPassword.equals(rawPassword) || storedPassword.equals(maHoaMatKhau(rawPassword));
    }

    private void nangCapMatKhauNeuCan(SQLiteDatabase db, long userId, String storedPassword, String rawPassword) {
        if (storedPassword == null || storedPassword.equals(maHoaMatKhau(rawPassword))) {
            return;
        }
        ContentValues v = new ContentValues();
        v.put(DBHelper.COL_PASSWORD, maHoaMatKhau(rawPassword));
        db.update(DBHelper.TBL_USERS, v, DBHelper.COL_ID + "=?", new String[]{String.valueOf(userId)});
    }

    private String maHoaMatKhau(String rawPassword) {
        try {
    ...
    private NguoiDung docNguoiDungThongKe(Cursor c) {
        NguoiDung user = docNguoiDung(c);
