...
import com.example.laptopshop.ui.admin.AdminCustomersActivity;
import com.example.laptopshop.ui.admin.AdminProductsActivity;
import com.example.laptopshop.ui.admin.AdminReportsActivity;
...
import com.example.laptopshop.ui.profile.ProfileActivity;
...
public abstract class BaseHomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (toolbar != null) {
            ...
            // Adjust title padding for Admin theme to prevent text clipping
            ...
            toolbar.setTitle(""); // Hide default title
            setSupportActionBar(toolbar);
        }

        TextView tvShellTitle = findViewById(R.id.tvShellToolbarTitle);
        if (tvShellTitle != null) {
            tvShellTitle.setText(screenTitle());
        }
        
        if (getSupportActionBar() != null) {
            getSupportActionBar().setDisplayHomeAsUpEnabled(shouldShowBackButton());
            if (toolbar != null && (shouldShowBackButton() || shouldUseAdminBackButtonStyling())) {
                toolbar.post(() -> {
                    Drawable upArrow = toolbar.getNavigationIcon();
                    if (upArrow != null) {
                        // Always white for the blue admin header
                        int tintColor = getColor(android.R.color.white);
                        upArrow.setTint(tintColor);
                    }
                });
            }
        }

        // Global Admin layout standardization removed to allow custom layouts
        // Margin should be handled in XML
    ...
    protected void onResume() {
        ...
        // Refresh title for shell-based headers
        TextView tvShellTitle = findViewById(R.id.tvShellToolbarTitle);
        if (tvShellTitle != null) {
            tvShellTitle.setText(screenTitle());
        }
    ...
    private void bindCustomerHomeHeader() {
        ...
        if (btnNotification != null) {
            btnNotification.setOnClickListener(v -> Toast.makeText(this, "Tính năng thông báo đang được phát triển", Toast.LENGTH_SHORT).show());
        }

        String fallbackName = session == null ? "LaptopShop" : session.getUsername();
        String displayName = fallbackName;
        if (session != null && session.getUserId() > 0) {
            NguoiDung user = new NguoiDungDao(this).getById(session.getUserId());
            if (user != null) {
        ...
        if (displayName == null || displayName.trim().isEmpty()) {
            displayName = "LaptopShop";
        }
        if ("Administrator".equalsIgnoreCase(displayName)) {
            displayName = "Admin";
        }
    ...
    private void setupHomeBrandClicks() {
        ...
        View tvApple = findViewById(R.id.tvBrandApple);
        if (tvApple != null) tvApple.setOnClickListener(brandClick);

        View tvDell = findViewById(R.id.tvBrandDell);
        if (tvDell != null) tvDell.setOnClickListener(brandClick);

        View tvAsus = findViewById(R.id.tvBrandAsus);
        if (tvAsus != null) tvAsus.setOnClickListener(brandClick);

        View tvLenovo = findViewById(R.id.tvBrandLenovo);
        if (tvLenovo != null) tvLenovo.setOnClickListener(brandClick);

        View tvHP = findViewById(R.id.tvBrandHP);
        if (tvHP != null) tvHP.setOnClickListener(brandClick);

        View tvMSI = findViewById(R.id.tvBrandMSI);
        if (tvMSI != null) tvMSI.setOnClickListener(brandClick);
    }
    ...
    private void updateHomeSearchOffset(int scrollY) {
        // Disabled to allow natural scrolling of header and search bar
    }
