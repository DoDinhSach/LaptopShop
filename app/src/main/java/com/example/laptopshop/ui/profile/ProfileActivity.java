package com.example.laptopshop.ui.profile;
...
public class ProfileActivity extends BaseHomeActivity {
    ...
    protected int shellLayoutRes() {
        return isAdminSession() ? R.layout.activity_home_bottom_admin_dashboard : R.layout.activity_home_bottom_customer;
    }
    ...
    protected void onResume() {
        super.onResume();
        if (nguoiDungDao != null && donHangDao != null && userId > 0) {
            loadProfile();
        }
    }
    ...
    private void bindActions() {
        if (itemOrdersOverview != null) itemOrdersOverview.setOnClickListener(v -> startActivity(new Intent(this, OrdersActivity.class)));
        if (itemEditProfile != null) itemEditProfile.setOnClickListener(v -> showEditProfileDialog());
        if (itemChangePassword != null) itemChangePassword.setOnClickListener(v -> showChangePasswordDialog());
        if (itemLogout != null) itemLogout.setOnClickListener(v -> confirmLogout());
    }
    ...
    private void loadProfile() {
        ...
        if (tvFullName != null) tvFullName.setText(valueOrFallback(user.fullname, user.username));
        if (tvUsername != null) tvUsername.setText(getString(R.string.profile_username_handle, user.username));

        ArrayList<DonHang> orders = donHangDao.getOrdersByUser(userId);
        ...
        if (tvOrdersCount != null) tvOrdersCount.setText(String.valueOf(orders.size()));
        if (tvProcessingCount != null) tvProcessingCount.setText(String.valueOf(processingCount));
        if (tvSpendTotal != null) tvSpendTotal.setText(formatCompactMoney(deliveredSpend));
    }
    ...
    private void saveFullName(String fullname) {
        ...
        boolean ok = nguoiDungDao.capNhatHoTen(userId, fullname);
        if (!ok) {
    ...
    private void savePassword(String oldPass, String newPass, String confirm) {
        ...
        boolean ok = nguoiDungDao.doiMatKhau(userId, oldPass, newPass);
        if (!ok) {
