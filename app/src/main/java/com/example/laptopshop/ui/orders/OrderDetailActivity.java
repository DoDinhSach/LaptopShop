...
import com.example.laptopshop.ui.home.AdminOrdersActivity;
...
public class OrderDetailActivity extends BaseHomeActivity {
    ...
    protected int shellLayoutRes() {
        return isAdminDetail() ? R.layout.activity_home_bottom_admin : R.layout.activity_home_bottom_customer;
    }
    ...
    private void loadOrder(boolean shouldScrollToStatus) {
        ...
        DonHang order = donHangDao.getOrderById(orderId);
        if (order == null) {
            Toast.makeText(this, R.string.order_not_found, Toast.LENGTH_SHORT).show();
            finish();
            return;
        ...
        ArrayList<DonHangItem> items = donHangDao.getOrderItems(orderId);
        if (adapter != null) {
            adapter.setData(items);
        }
        bindOrder(order, items);

        if (shouldScrollToStatus && isAdminDetail() && cardStatusUpdate != null && cardStatusUpdate.getVisibility() == View.VISIBLE) {
            scrollToStatusSection();
        }
    }

    private void bindOrder(DonHang order, ArrayList<DonHangItem> items) {
        ...
        if (tvOrderCode != null) tvOrderCode.setText(getString(R.string.admin_order_code, order.id));
        if (tvOrderDate != null) tvOrderDate.setText(formatDate(order.ngayTao));
        if (tvOrderMeta != null) {
        ...
        if (tvCustomerName != null) tvCustomerName.setText(getString(R.string.order_customer_name) + " " + valueOrDash(order.nguoiNhan));
        if (tvCustomerPhone != null) tvCustomerPhone.setText(getString(R.string.order_customer_phone) + " " + valueOrDash(order.sdtNhan));
        if (tvCustomerAddress != null) tvCustomerAddress.setText(getString(R.string.order_customer_address) + " " + valueOrDash(order.diaChiNhan));

        if (tvCustomerNote != null) {
        ...
        if (tvProductsSummary != null) tvProductsSummary.setText(getString(R.string.order_products_summary, items.size()));
        if (tvOrderSubtotal != null) tvOrderSubtotal.setText(formatCurrency(subtotal));
        if (tvOrderShippingFee != null) tvOrderShippingFee.setText(formatCurrency(shippingFee));
        if (tvOrderDiscount != null) tvOrderDiscount.setText(getString(R.string.order_discount_value, formatCurrency(discountAmount), valueOrDash(order.maGiamGia)));
        if (tvOrderTotal != null) tvOrderTotal.setText(formatCurrency(total));
        if (tvPaymentMethod != null) tvPaymentMethod.setText(formatPaymentMethod(order.phuongThucThanhToan));
        if (tvPaymentStatusOverview != null) tvPaymentStatusOverview.setText(getString(
        ...
        if (tvOrderStatusCurrent != null) tvOrderStatusCurrent.setText(getString(R.string.order_status_current, OrdersAdapter.formatOrderStatus(this, order.trangThaiDon)));
        if (tvPaymentStatusCurrent != null) tvPaymentStatusCurrent.setText(getString(R.string.payment_status_current, OrdersAdapter.formatPaymentStatus(this, order.trangThaiThanhToan)));
    ...
    private void applyStatusUpdates(String nextOrderStatus, String nextPaymentStatus) {
        ...
        if (nextPaymentStatus != null && !codDeliveryWillMarkPaid) {
            boolean ok = donHangDao.updatePaymentStatus(orderId, nextPaymentStatus);
            if (!ok) {
    ...
    private boolean updateOrderStatusToTarget(String targetStatus) {
        ...
        if (OrderStatus.STATUS_DA_GIAO.equals(targetStatus)
            ...
            if (!donHangDao.updateOrderStatus(orderId, OrderStatus.STATUS_DANG_XU_LY)) {
                return false;
            }
            order = donHangDao.getOrderById(orderId);
            return order != null && donHangDao.updateOrderStatus(orderId, OrderStatus.STATUS_DA_GIAO);
        }

        return donHangDao.updateOrderStatus(orderId, targetStatus);
    }

    private void bindStatusControls(DonHang order) {
        if (!isAdminDetail()) {
            if (cardStatusUpdate != null) cardStatusUpdate.setVisibility(View.GONE);
            return;
        }

        if (cardStatusUpdate != null) cardStatusUpdate.setVisibility(View.VISIBLE);
        availableOrderStatuses.clear();
        availablePaymentStatuses.clear();
        availableOrderStatuses.add(order.trangThaiDon);
        availableOrderStatuses.addAll(donHangDao.getAllowedNextOrderStatuses(order));
        ...
        availablePaymentStatuses.addAll(donHangDao.getAllowedNextPaymentStatuses(order));
        ...
        boolean finalState = donHangDao.isFinalOrderStatus(order.trangThaiDon);
        ...
        if (layoutOrderStatusAction != null) layoutOrderStatusAction.setVisibility(!finalState ? View.VISIBLE : View.GONE);
        if (layoutPaymentStatusAction != null) layoutPaymentStatusAction.setVisibility(!finalState ? View.VISIBLE : View.GONE);
        if (btnUpdateStatus != null) {
    ...
    private void bindActionHeader(DonHang order, boolean finalState, boolean hasOrderActions, boolean hasPaymentActions) {
        if (tvActionHeadline == null || tvActionSummary == null) return;
        if (finalState) {
    ...
    private boolean canCompleteFromPendingInOneAction(DonHang order) {
        ...
        return PaymentStatus.STATUS_DA_THANH_TOAN.equals(order.trangThaiThanhToan)
                || donHangDao.canTransitionPayment(order, PaymentStatus.STATUS_DA_THANH_TOAN);
    }

    private void bindCompletedState(DonHang order, boolean finalState) {
        if (cardStatusCompleted == null) return;
        if (!finalState) {
        ...
        if (OrderStatus.STATUS_DA_GIAO.equals(order.trangThaiDon)) {
            if (tvStatusCompletedTitle != null) tvStatusCompletedTitle.setText(R.string.order_admin_completed_card_title);
            if (tvStatusCompletedMessage != null) tvStatusCompletedMessage.setText(getString(
                    R.string.order_admin_completed_card_message,
                    OrdersAdapter.formatPaymentStatus(this, order.trangThaiThanhToan)
            ));
        } else {
            if (tvStatusCompletedTitle != null) tvStatusCompletedTitle.setText(R.string.order_admin_cancelled_card_title);
            if (tvStatusCompletedMessage != null) tvStatusCompletedMessage.setText(isBankTransfer(order)
    ...
    private void bindOrderStatusControls(DonHang order, boolean enabled) {
        if (spinnerOrderStatus == null) return;
        ...
        if (tvOrderActionHint != null) tvOrderActionHint.setText(getOrderActionHint(order, enabled));
    }

    private void bindPaymentStatusControls(DonHang order, boolean enabled) {
        if (spinnerPaymentStatus == null) return;
        ArrayList<String> labels = new ArrayList<>();
        ...
        if (tvPaymentActionHint != null) tvPaymentActionHint.setText(getPaymentActionHint(order, enabled));
    }

    private String getOrderActionHint(DonHang order, boolean enabled) {
        if (enabled) {
            if (OrderStatus.STATUS_CHO_XAC_NHAN.equals(order.trangThaiDon)) {
        ...
        if (donHangDao.isFinalOrderStatus(order.trangThaiDon)) {
    ...
    private String getPaymentActionHint(DonHang order, boolean enabled) {
        if (enabled) {
    ...
    private void scrollToStatusSection() {
        if (scrollViewOrderDetail != null && cardStatusUpdate != null) {
    ...
    private void bindTimeline(DonHang order) {
        bindTimelineStep(
    ...
    private int calculateTotal(ArrayList<DonHangItem> items) {
        int total = 0;
        for (DonHangItem item : items) {
    ...
    private String buildOrderHeaderSummary(ArrayList<DonHangItem> items) {
    ...
    private void bindPaymentLifecycleInfo(DonHang order) {
    ...
    private void bindRefundInfo(DonHang order) {
    ...
    private String buildPaymentLifecycleInfo(DonHang order) {
    ...
    private String buildRefundInfo(DonHang order) {
        if (order == null || order.refundStatus == null || order.refundStatus.trim().isEmpty()) {
            return null;
        }
        if (DonHang.REFUND_STATUS_CHO_HOAN_TIEN.equals(order.refundStatus)) {
        ...
        if (DonHang.REFUND_STATUS_DA_HOAN_TIEN.equals(order.refundStatus)) {
    ...
    private boolean isCod(DonHang order) {
        return order != null && CheckoutInfo.PAYMENT_COD.equals(order.phuongThucThanhToan);
    }

    private boolean isBankTransfer(DonHang order) {
