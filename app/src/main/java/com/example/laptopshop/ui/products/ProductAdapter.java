...
public class ProductAdapter extends RecyclerView.Adapter<ProductAdapter.VH> {
    ...
    private final ArrayList<SanPham> data = new ArrayList<>();
    ...
    public void setData(ArrayList<SanPham> newData) {
        data.clear();
    ...
    private void bindDefaultCard(VH holder, SanPham p, String hang) {
        if (hang.isEmpty()) {
    ...
    private void bindFlashSaleCard(VH holder, SanPham p) {
        bindCompactCard(holder, p, true, true, true);
    }

    private void bindCatalogCompactCard(VH holder, SanPham p) {
        bindCompactCard(holder, p, false, true, true);
    }

    private void bindCompactCard(VH holder, SanPham p, boolean saleBadgeMode, boolean showMeta, boolean customerStyle) {
        holder.tvBrand.setVisibility(View.GONE);
        holder.layoutLegacyMeta.setVisibility(View.GONE);
        holder.tvOs.setVisibility(View.GONE);
    ...
    private String buildSpecsText(SanPham p) {
    ...
    private String buildFlashSaleSpecsText(SanPham p) {
        ArrayList<String> parts = new ArrayList<>();
    ...
    private String buildFlashSaleBadgeText(SanPham p) {
        String brand = p.hang == null ? "" : p.hang.trim();
        return brand.isEmpty() ? "Flash deal" : "Hãng " + brand;
    }

    private String buildCatalogCompactBadgeText(SanPham p) {
        int stock = p.tonKho;
    ...
    private String buildRatingText(SanPham p) {
        float rating = p.danhGia > 0 ? p.danhGia : 4.8f;
        return "★ " + String.format(Locale.getDefault(), "%.1f", rating);
    }

    private String buildSoldText(SanPham p) {
